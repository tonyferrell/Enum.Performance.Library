properties {
  ...
  $builds = @(
    @{Name = "Enum.Performance.Library"; TestsName = ""; BuildFunction = "MSBuildBuild"; TestsFunction = ""; Constants=""; FinalDir="Net45"; NuGetDir = "net45"; Framework="net-4.0"},
    @{Name = "Enum.Performance.Library.Net40"; TestsName = ""; BuildFunction = "MSBuildBuild"; TestsFunction = ""; Constants="NET40"; FinalDir="Net40"; NuGetDir = "net40"; Framework="net-4.0"},
    @{Name = "Enum.Performance.Library.Net35"; TestsName = ""; BuildFunction = "MSBuildBuild"; TestsFunction = ""; Constants="NET35"; FinalDir="Net35"; NuGetDir = "net35"; Framework="net-2.0"},
    @{Name = "Enum.Performance.Library.Net20"; TestsName = ""; BuildFunction = "MSBuildBuild"; TestsFunction = ""; Constants="NET20"; FinalDir="Net20"; NuGetDir = "net20"; Framework="net-2.0"}
  )
}
...
task ConstructNuget -depends Build {
  ...
  $nuspecPath = "$workingDir\NuGet\Enum.Performance.Library.nuspec"
  Copy-Item -Path "$buildDir\Enum.Performance.Library.nuspec" -Destination $nuspecPath -recurse
  ...
  foreach ($build in $builds)
  {
    if ($build.NuGetDir)
    {
      ...
      {
        robocopy "$workingSourceDir\Enum.Performance.Library\bin\Release\$finalDir" $workingDir\NuGet\lib\$frameworkDir *.dll *.pdb *.xml /NFL /NDL /NJS /NC /NS /NP /XO /XF *.CodeAnalysisLog.xml | Out-Default
      }
    }
  }
...
# Run tests
task Test -depends ConstructNuget {
  foreach ($build in $builds)
  {
    if ($build.TestsFunction)
    {
